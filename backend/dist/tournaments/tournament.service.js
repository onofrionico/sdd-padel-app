    ...
    async findAll(options = {}) {
        ...
        if (options.where) {
            ...
            if (options.where.category) {
                queryBuilder.andWhere("tournament.settings::jsonb->'categoryRange'->>'min' <= :category AND tournament.settings::jsonb->'categoryRange'->>'max' >= :category", { category: options.where.category });
            }
            if (options.where.search) {
                queryBuilder.andWhere('(LOWER(tournament.name) LIKE LOWER(:search) OR LOWER(tournament.description) LIKE LOWER(:search))', { search: `%${options.where.search}%` });
            }
        }
